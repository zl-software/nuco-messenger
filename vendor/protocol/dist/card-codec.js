...
import { CONTACT_CARD_VERSION, CARD_HANDLE_MAX_LEN, CARD_NAME_MAX_LEN, CARD_SERVER_MAX_LEN, isContactCard, } from './qr.js';
import { IDENTITY_KEY_LEN, SIGNED_PREKEY_PUB_LEN, KYBER_PREKEY_PUB_LEN, PREKEY_SIGNATURE_LEN, } from './prekeys.js';
...
function pushHeader(out, major, len) {
    const base = major << 5;
    if (len < 24)
        out.push(base | len);
    else if (len < 256)
        out.push(base | 24, len);
    else if (len < 65536)
        out.push(base | 25, len >> 8, len & 0xff);
    else
        out.push(base | 26, (len / 16777216) & 0xff, (len >> 16) & 0xff, (len >> 8) & 0xff, len & 0xff);
}
function pushUint(out, value) {
...
class CborReader {
    ...
    header(expectedMajor) {
        const first = this.byte();
        if (first >> 5 !== expectedMajor)
            throw new Error('major mismatch');
        const info = first & 31;
        if (info < 24)
            return info;
        if (info === 24) {
    ...
    text(maxLen) {
        ...
        if (len > maxLen * 4)
            throw new Error('text too long'); // bytes, chars checked after decode
        if (this.pos + len > this.data.length)
            throw new Error('truncated');
...
function encodeCardBytes(card) {
    ...
    if (identityKey?.length !== IDENTITY_KEY_LEN ||
    ...
    if (card.server === undefined)
        out.push(CBOR_NULL);
    else
        pushText(out, card.server);
...
// untrusted input.
export function encodeContactCardQr(card) {
...
// exact re-encode).
export function decodeContactCardQr(data) {
    if (!data.startsWith(CARD_QR_PREFIX))
        return null;
    ...
    try {
        ...
        if (reader.arrayHeader() !== CARD_FIELD_COUNT)
            return null;
        const v = reader.uint(UINT_MAX);
        if (v !== CONTACT_CARD_VERSION)
            return null;
...
//# sourceMappingURL=card-codec.js.map